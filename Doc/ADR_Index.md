...
|---|---|---|---|---|
| [ADR-001](adr/ADR-001-mirror-pict-bnf-subset.yaml) | PICT BNF を最小サブセットとして mirror する | 2026-05-02 | accepted | PICT BNF を 1:1 で mirror するが、MVP 採用は最小サブセットに限定する |
| [ADR-002](adr/ADR-002-invoke-pict-as-external-cli.yaml) | PICT を外部 CLI として呼び出し、バンドルしない | 2026-05-02 | accepted | PICT を外部 CLI として実行時に呼び出し、NeoCombi のバンドルには含めない |
| [ADR-003](adr/ADR-003-no-embedded-ai-n8n-passthrough.yaml) | AI を内蔵せず、連携は n8n 経由のパススルーに限定する | 2026-05-02 | accepted | AI ロジックは NeoCombi に内蔵せず、n8n を介した外部パススルーに限定する |
| [ADR-004](adr/ADR-004-cli-and-gui-dual-modes.yaml) | CLI と GUI の二刀流（エンジン層を共有） | 2026-05-02 | accepted | GUI と CLI の両方を提供。共通エンジン層（DSL parser / evaluator / PICT 連携）を共有する |
| [ADR-005](adr/ADR-005-builtin-dsl-evaluator.yaml) | DSL 評価器を内製し、禁則ビューを PICT 非依存で計算する | 2026-05-02 | accepted | DSL の semantic evaluator を NeoCombi 自身に実装し、禁則表は内製評価器のみで計算する |
...
## ADR-012 — 禁則判定の自由因子列挙は『制約到達可能閉包』に絞る
...
isPartiallyForbidden は「ある部分割当が禁則であるか」を判定するため、部分割当に含まれない自由因子全てに対して水準 Cartesian 積を全展開し、どれか一つでも全制約を満たす全割当が存在するかを探していた。100 因子規模では自由因子が ~98 個、各 3〜5 水準 → 列挙総数 ~4^98 ≈ 10^59 に達し、DEFAULT_ENUMERATION_LIMIT (1,000,000) で必ず too-large 失敗する。結果、HAYST 100 因子モデルでは Forbidden ビューが事実上使えなかった。

### Decision

**部分割当の因子から「同一制約内に共起する」関係で到達可能な因子の閉包だけを列挙対象にする**

isPartiallyForbidden を以下のアルゴリズムに置き換えた：

1. 部分割当のキー集合 seed から出発し、「ある制約に共起する因子は全て同一到達集合に追加」を不動点まで反復して到達閉包 R を得る。
2. 自由因子のうち R に属するもののみを列挙対象とする。R の外の因子は slice の禁則性に影響しないので、既定値を選ぶ必要すらなく無視する。
3. 制約のうち、その因子集合が R と非空交差するもののみをチェック対象とする（到達性の定義から、R に触れる制約はその因子全てが R 内になる）。
4. 自由因子集合の Cartesian 積カーディナリティが limit を超える場合のみ too-large を返す。R が空なら即座に「禁則ではない」を返す。

アルゴリズム正当性：R 外の因子は seed と制約共起グラフ上で連結していないので、それらの値は seed と独立に選べる。モデル全体が大域的に充足可能である限り、R 外の因子に対する充足割当は必ず存在し、slice の禁則性判定に影響しない。R 外の制約も同様に、slice と独立な部分モデルの問題として、本判定では検査不要。モデルが大域的に非充足の場合、本判定は誤って「禁則でない」と返す可能性が残る（false-negative）。これは MVP では受容する。

### Neglected Options

- **総当たり列挙のまま limit を 100,000,000 へ大きく緩める** — 100 因子規模では 4^98 オーダーで limit を 10x にしてもオーバー。本質的に間に合わない
- **外部 SAT ソルバ / CSP ソルバを内蔵** — ADR-003（AI/外部依存を持ち込まない）に反する、依存ライセンス審査が増える、決定論的かつ簡素という MVP 方針と齟齬
- **PICT に判定を委ねる（PICT 出力に禁則情報を抽出させる）** — ADR-005 の『Forbidden は PICT 非依存で内製』を覆す。PICT の出力には part-of-test-case か否かしか出ないため流用も難しい
- **到達閉包は使わず、slice 因子と直接共起する因子のみ列挙（1-hop）** — A → B → C の伝播禁則を取りこぼす。propagation slice (ADR-011) と整合せず、ユーザの提案通りの slice を計算できない
- **列挙ではなく Constraint Propagation (e.g., AC-3) でドメイン削減** — 実装複雑度が大きい。MVP の評価器を全面書き換えになる。reachability scoping だけで十分高速になるため、必要になってから検討

### Consequences

**Positive:**
- 100 因子モデルでも slice の禁則計算が現実的時間で完了する（典型ケースで成分サイズ ~5、列挙 ~5^4 ≈ 625）
- 既存 evaluator の構造を保つ（enumerateExtensions / isConstraintSatisfied は無変更）
- PICT 非依存・決定論的・依存ライブラリゼロ
- サイズ制限ガードは残るので、密結合な大規模成分が来た場合の防御は保たれる

**Negative:**
- モデルが大域的に非充足な場合、R 外で起きる矛盾を本判定では検出できない（false-negative）
- アルゴリズムの説明・テストが少し複雑化した

**Risks:**
- constraint co-occurrence が密で全因子が一成分になると、改善前と同じく爆発する。100 因子全結合なら 4^99 で再び too-large。実用 HAYST モデルでは稀と推定するが将来の保護が必要になる可能性
- false-negative リスクをユーザに気づかせる UI 表示が現状ない（PICT 経由で気づくのを期待）

### References

- Doc/requirements/system_requirements.yaml SR-030
- ADR-005
- ADR-011
