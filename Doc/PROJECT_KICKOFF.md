...
- DSL は **PICT 制約言語を 1:1 で mirror**（ユーザは PICT 公式ドキュメントをそのまま参照可能。MVP は §4 のサブセットに限定）

### 最終ビジョン（究極の狙い）

NeoCombi の "完成形" は **自然言語要求仕様 → AI → PICT DSL → 人間レビュー → 完成** のオーサリングフロー。下ペーンの「自然言語要求仕様」入力欄にテスト対象の振る舞いを自然文で書くと、n8n 経由で AI が PICT DSL を生成し、人間がレビュー＆修正して完成させる。

ModelLogue が「既存モデルに AI レビューをかける」のに対し、NeoCombi は **AI 生成 → 人間レビュー** のフロー。方向は逆。

MVP では AI 連携機能は範囲外（n8n 通信仕様も保留）。だが下ペーンのレイアウトとデータモデルは、この最終ビジョンを前提に設計する（後述 §5 / §8）。
...
### mirror する範囲（MVP は最小サブセット）

MVP では **PICT 制約言語の最小サブセット** を mirror する。代替表現で同じことを書ける冗長な構文は MVP から落とし、必要なら v2 以降で復活検討。EBNF は [`DSL_Grammar_Specification.md`](DSL_Grammar_Specification.md) で独立定義し、PICT BNF からの差分（捨てた要素）を明示する。

採用する（MVP）:
- 制約節：`IF ... THEN ... [ELSE ...]`
- 比較演算子：`=` `<>` `>` `>=` `<` `<=`
- 論理演算子：`AND` `OR` `NOT`
- 集合：`IN { ... }`（OR 連鎖の代替表現だが、AI 親和性のため採用）
- パラメータ宣言（因子・水準の宣言文）

MVP では落とす（v2 以降で再検討）:
- パターン：`LIKE "..."`（ワイルドカードマッチ。実装コスト高）
- サブモデル（`{f1, f2} @ N`）／weight／negative value（`~`）：いずれも制約意味論ではなく PICT の生成方略チューニング。内製評価器の責務外

### NeoCombi 内製 DSL 評価器

DSL の役割は「PICT 入力」だけではない。**禁則の参照ビュー（多項間禁則表）を NeoCombi 単独で計算する** ためのインプットでもある。これにより：

- PICT を起動しなくても、編集中に即時で N 因子の禁則組合せを可視化できる
- 決定論性：PICT 結果に依存せず、同じ DSL → 同じ禁則集合
- レスポンス性：プロセス起動・ファイル I/O を待たない

責務分担:
- **NeoCombi 内製評価器**：DSL → 任意 N 因子の禁則組合せ列挙 → 多項間禁則表ビュー
- **PICT 外部 CLI**：DSL → pairwise/N-wise テストケース生成

評価器の対応範囲は上記 MVP サブセットと一致。`LIKE` やサブモデル・weight・negative は評価器の対象外。
...
「AI 内蔵しない」原則は維持しつつ、**n8n 経由の AI 連携機能** をオプションで提供する。これは ModelLogue と同じ方式であり、ModelLogue 自身も AI ロジックを内蔵せず n8n でパススルーしている。

連携モードの挙動:
- **連携 OFF**：完全単独動作（NeoCEG と区別がつかない外観）
- **連携 ON**：UI 右ペーンにチャットを表示し、下ペーンに「自然言語要求仕様」タブが追加される（§1 最終ビジョン参照）。入力は n8n エンドポイントへ送信

この設計により、AI 連携の有無で本体ロジック（決定論変換器の責務）は変わらない。AI 機能を抜き差ししても CI/CD 利用に影響しない。

n8n の通信仕様は MVP では保留し、ModelLogue の連携実装が成熟したら追従する。
